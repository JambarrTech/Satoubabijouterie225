    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0B5D1E]/10 text-[#0B5D1E] text-xs font-semibold">
          ...
          <span>Service Après-Vente & Restauration Satouba Bijouterie 255</span>
        </div>
      ...
      {success ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-gray-100 p-8 space-y-6 shadow-sm">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Photos du bijou (optionnel)</label>
            <div className="flex flex-wrap gap-3">
              {photos.map((img, idx) => (
                <div key={idx} className="relative w-20 h-20 rounded-xl overflow-hidden border border-gray-200">
                  <img src={img} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  <button type="button" onClick={() => setPhotos(prev => prev.filter((_, i) => i !== idx))} className="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full text-xs">×</button>
                </div>
              ))}
              <label className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-300 hover:border-[#0B5D1E] flex flex-col items-center justify-center text-gray-400 hover:text-[#0B5D1E] transition-colors cursor-pointer">
                {uploading ? <div className="w-4 h-4 border-2 border-[#0B5D1E] border-t-transparent rounded-full animate-spin" /> : <span className="text-[10px]">+ Photo</span>}
                <input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={handlePhotoUpload} className="hidden" />
              </label>
            </div>
            <p className="text-[10px] text-gray-400 mt-1">JPG, PNG ou WebP — max 5 Mo</p>
          </div>
