...
export function CustomView() {
  ...
  useEffect(() => {
    fetchMaterialPricing().then(setPricing).catch(console.error);
  }, []);

  // Real-time price estimator calculation logic
  const estimatedPrice = useMemo(() => {
    const materials = pricing.filter(p => p.type === 'material');
    const stones = pricing.filter(p => p.type === 'stone');
    const labor = pricing.find(p => p.type === 'labor');

    const materialEntry = materials.find(p => p.name === material);
    const baseRatePerGram = materialEntry ? materialEntry.pricePerGram : 45000;

    const stoneEntry = stones.find(p => p.name === stone);
    const stoneCost = stoneEntry ? stoneEntry.pricePerGram : 0;

    const laborFee = labor ? labor.pricePerGram : 45000;

    const total = (weight * baseRatePerGram) + stoneCost + laborFee;
    return Math.round(total / 5000) * 5000; // round to nearest 5k
  }, [material, weight, stone, pricing]);
