    ...
    >
      <div className="relative aspect-square bg-gray-50 overflow-hidden">
        ...
        >
          <Heart
            size={18}
            className={`transition-all duration-200 ${isFavorite ? "fill-red-500 text-red-500 scale-110" : "text-gray-600"}`}
          />
        </motion.button>

        {/* Add to cart button — slides up on hover */}
        <div className="absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-10">
          <motion.button
            onClick={handleAdd}
            whileTap={{ scale: 0.95 }}
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        ...
        <div className="mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
          ...
          <LikeButton productId={product.id} initialLikesCount={product.likesCount} size="sm" />
        </div>
      </div>
    </motion.div>
  );
